        ...
        <CardContent className="p-3 sm:p-2">
          ...
          <div className="space-y-2 sm:space-y-1">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-1 text-xs">
              <span className="text-black px-2 py-1 sm:px-1 sm:py-0.5 rounded text-xs font-medium" style={{background: 'linear-gradient(135deg, rgb(251 191 36) 0%, rgb(245 158 11) 50%, rgb(217 119 6) 100%)'}}>${(parseFloat(product.tierPricing?.bronze?.msrp || product.price?.msrp || product.priceBronze || "0")).toFixed(2)}</span>
              <span className="text-black px-2 py-1 sm:px-1 sm:py-0.5 rounded text-xs font-medium" style={{background: 'linear-gradient(135deg, rgb(254 240 138) 0%, rgb(250 204 21) 50%, rgb(234 179 8) 100%)'}}>${(parseFloat(product.tierPricing?.gold?.dealerPrice || product.price?.dealerPrice || product.priceGold || "0")).toFixed(2)}</span>
              <span className="text-black px-2 py-1 sm:px-1 sm:py-0.5 rounded text-xs font-medium" style={{background: 'linear-gradient(135deg, rgb(209 213 219) 0%, rgb(156 163 175) 50%, rgb(107 114 128) 100%)'}}>${(parseFloat(product.tierPricing?.platinum?.dealerCasePrice || product.price?.dealerCasePrice || product.pricePlatinum || "0")).toFixed(2).replace(/\d/g, '*')}</span>
            </div>
          </div>
        </CardContent>
      </Card>
